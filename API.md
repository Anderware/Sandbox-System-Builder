...
Removes a property from an actors data.  If the actor does not have the property, nothing will be done.

Warning: This can not be undone, only use this function if you know what you are doing.
...
let selectedActor = api.Actor_GetFromSelectedToken();
if (selectedActor!=null){
  	api.ActorProperty_RemoveProperty(selectedActor, propertyKey)           
}
```
