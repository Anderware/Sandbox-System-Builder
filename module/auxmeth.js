...
export class auxMeth {
    ...
    static async importTree(exportfilePath) {
        ...
        for (let i = 0; i < actors.length; i++) {
            ...
            let istemplate = foundry.utils.duplicate(anactor.system.istemplate);
            // -------------------------------------------
        ...
        for (let folder of game.folders.contents) {
            if (foundry.utils.hasProperty(idCollection, folder.realparent))
        ...
        for (let item of game.items.contents) {
            let finalitem = await foundry.utils.duplicate(item);
            //console.log("importing item: " + finalitem.name);

            if (item.type == "property") {
                if (foundry.utils.hasProperty(idCollection, finalitem.dialogID) && finalitem.system.dialogID != "")
                    finalitem.system.dialogID = idCollection[finalitem.system.dialogID];

                if (foundry.utils.hasProperty(idCollection, finalitem.system.group.id) && finalitem.system.group.id != "")
            ...
            if (item.type == "panel" || item.type == "group") {
                if (finalitem.system.properties.length > 0) {
                    for (let property of finalitem.system.properties) {
                        if (foundry.utils.hasProperty(idCollection, property.id)) {
            ...
            if (item.type == "multipanel" || item.type == "sheettab") {
                if (finalitem.system.panels.length > 0) {
                    for (let panel of finalitem.system.panels) {
                        if (foundry.utils.hasProperty(idCollection, panel.id))
            ...
            if (item.type == "cItem") {
                if (finalitem.system.groups.length > 0) {
                    for (let group of finalitem.system.groups) {
                        if (foundry.utils.hasProperty(idCollection, group.id)) {
                    ...
                    for (let mod of finalitem.system.mods) {
                        if (mod.items.length > 0) {
                            for (let moditem of mod.items) {
                                if (foundry.utils.hasProperty(idCollection, moditem.id)) {
        ...
        for (let actor of game.actors.contents) {
            let finalactor = await foundry.utils.duplicate(actor);
            //console.log("importing actor: " + finalactor.name);
            if (finalactor.token != null)
                if (finalactor.token.actorId != null)
                    if (foundry.utils.hasProperty(idCollection, finalactor.token.actorId))
            ...
            if (actor.system.citems.length > 0)
                for (let citem of finalactor.system.citems) {
                    if (foundry.utils.hasProperty(idCollection, citem.id)) {
                        ...
                        if (citem.mods)
                            for (let mod of citem.mods) {
                                if (foundry.utils.hasProperty(idCollection, mod.citem))
            ...
            if (actor.system.tabs.length > 0)
                for (let tab of finalactor.system.tabs) {
                    if (foundry.utils.hasProperty(idCollection, tab.id)) {
            ...
            for (var key in finalactor.system.attributes) {
                if (finalactor.system.attributes[key].id != null) {
                    if (foundry.utils.hasProperty(idCollection, finalactor.system.attributes[key].id))
            ...
            for (let i = 0; i < result.length; i++) {
                ...
                if (attributes != null) {
                    ...
                    if (myatt != null) {
                        ...
                        ui.notifications.warn("autoParser | Property " + rawattname + mycitem + " in expression[" + expr + "] does not exist");
  ...
  static async  useAPIFunction(functionName,expr){
    ...
    static aTdynamicSort(property, datatype) {
        ...
        return function (a, b) {
            ...
            * and you may want to customize it to your needs
            */
            if (foundry.utils.hasProperty(a.attributes[property], "value")) {
    ...
    static  GetConditionalText(valueToCompare, mappingString) {
      ...
      try{
        ...
        // Split on comma or pipe into "value:text" pairs
        const pairs = mappingString.split(/[|,]/);

        // Parse into { value: number, text: string }
        const thresholds = pairs
          .map(p => p.trim())
          .filter(p => p.length > 0)
          .map(p => {
            const [valuePart, ...textParts] = p.split(":");
            const value = Number(auxMeth.computeExpression(valuePart.trim())); 
            const text = textParts.join(":").trim(); // allow colons in text
            return Number.isFinite(value) && text ? { value, text } : null;
          })
          .filter(entry => entry !== null)
          .sort((a, b) => a.value - b.value); // ensure ascending order

        // Find the last threshold whose value <= valueToCompare
        
        for (const entry of thresholds) {
          if (valueToCompare >= entry.value) {
            result = entry.text;
          } else {
            break;
          }
    ...
    static computeExpression(expr) {
      ...
      // Very small wrapper around JS arithmetic
      // Supports + - * / % ** and parentheses as JavaScript does
      // WARNING: Only safe if `expr` is fully trusted!
    ...
    static async checkConsistency() {
        ...
        for (let i = 0; i < gamecItems.length; i++) {
            ...
            for (let j = 0; j < mycitemmods.length; j++) {
                ...
                //foundry.utils.setProperty(mymod, "citem", mycitem.data.id);
                // if (!foundry.utils.hasProperty(mymod, "index")) {
                //     foundry.utils.setProperty(mymod, "index", j);
                //     toupdate = true;
                // }
        ...
        //                                 if (!foundry.utils.hasProperty(mymods[r], "index"))
        //                                     foundry.utils.setProperty(mymods[r], "index", 0);
