...
export class sItemSheet extends foundry.appv1.sheets.ItemSheet {
    ...
    async getData() {
        ...
        data.enrichedBiography = await foundry.applications.ux.TextEditor.implementation.enrichHTML(this.item.system.description, {secrets:secrets, entities:true,async: true});
        ...
        if (this.item.type == "cItem"){
          data.citem={
            usetypes:{
              PAS: game.i18n.localize("SANDBOX.cItem.ItemAttribute.UseType.Passive.Caption"),
              ACT: game.i18n.localize("SANDBOX.cItem.ItemAttribute.UseType.Activation.Caption"),
              CON: game.i18n.localize("SANDBOX.cItem.ItemAttribute.UseType.Consumable.Caption")
            },
            icons:{
              BOOK:"BOOK",
              VIAL:"VIAL",
            ...
            modcondops:{
              NON:game.i18n.localize("SANDBOX.cItem.ItemAttribute.Mod.CondOp.None.Caption"),
              EQU:"=",
              HIH:">",
              LOW:"<"
            },
            modlistmods:{
              INCLUDE: game.i18n.localize("SANDBOX.cItem.ItemAttribute.Mod.ListMod.Include.Caption"),
              REMOVE:game.i18n.localize( "SANDBOX.cItem.ItemAttribute.Mod.ListMod.Remove.Caption")
            } 
          }
        }
        ...
        if (this.item.type == "panel" || this.item.type == "multipanel" ){
          data.panel={
            condops:{
              NON:"NONE",
              EQU:"=",
    ...
    async refreshCIAttributes(basehtml) {
        ...
        let newgroups = foundry.utils.duplicate(groups);
...
//                            new_row.setAttribute("id", i);
                            ...
                            if (property.datatype != "group" && property.datatype != "label") {
                                ...
                                if (!foundry.utils.hasProperty(attributes, property.attKey)) {
                                    foundry.utils.setProperty(attributes, property.attKey, {});
                                    let value='';
                                ...
                                if (property.datatype != "radio") {
                                  ...
                                  // check if user as permission to edit
                                  if (game.user.isGM || this.item.testUserPermission(game.user,CONST.DOCUMENT_OWNERSHIP_LEVELS.OWNER)) {
                                    // ok
                                    input.setAttribute("data-user-has-permission", "true");
                                  } else{
                                    input.setAttribute("readonly", "true");
                                  }
                                }
...
async upDateTabBodiesHeight(){
    ...
    async _updateObject(event, formData) {
      //console.log('Sandbox.sItemSheet._updateObject',event,formData)
