...
async function searchItems(searchForKey,search_options){
  ...
  for (let i = 0; i < items.length; i++) {
    if(search_options.itemName){
      await checkThisItemName(search_result,searchForKey,items[i],'ciKey','name','Name');
    }
    // check attributes
    let attributes = Object.getOwnPropertyNames(items[i].system.attributes);
    for(let attribute of attributes){
      if(attribute.includes(searchForKey)){
        let result={
        itemId:items[i].id,
        itemImg:items[i].img,
        itemName:items[i].name,
        itemType:items[i].type,
        itemKey:items[i].system['ciKey'],
        folder:items[i].folder?.name || '',
        fieldName:'Attribute',
        fieldValue:markFound(attribute,searchForKey,null)
      };
      search_result.push(result);
      }
    }
