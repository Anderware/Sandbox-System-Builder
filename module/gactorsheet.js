...
export class gActorSheet extends foundry.appv1.sheets.ActorSheet {
    ...
    async getData() {
        ...
        data.enrichedBiography = await foundry.applications.ux.TextEditor.implementation.enrichHTML(this.actor.system.biography, {secrets:secrets, entities:true,async: true});
    ...
    async _renderInner(data, options) {
        ...
        const html = template(foundry.utils.duplicate(data));
        this.form = $(html)[0];
    ...
    activateListeners(html) {
        ...
        let stabs = foundry.utils.duplicate(actor.system.tabs);
    ...
    */
                ...
                one: {
                    ...
                    callback: async (html) => {
                        ...
                        for (let k = 0; k < dialogvalues.length; k++) {
                            ...
                            foundry.utils.setProperty(dialogProps, myKey, {});
    ...
    async generateDialogPanelHTML(dialogPanel,dialogProps) {
        ...
        for (let i = 0; i < parseInt(dialogPanel.system.properties.length); i++) {
            ...
            // add to props
            foundry.utils.setProperty(dialogProps, panelPropertyRef.ikey, {});                         
    ...
    async setTemplate(gtemplate, actorData) {
        ...
        if (actorData == null)
            actorData = foundry.utils.duplicate(this.actor.system);
    ...
    async setAttributeValues(attID, attData, propName) {
        ...
        if (idkey == null) {
            ...
            //                console.log("setting");
            //                foundry.utils.setProperty(attData[attribute],"maxblocked", true);
            //            }
            //            else{
            //                foundry.utils.setProperty(attData[attribute],"maxblocked", false);
            //            }

        }

        if (property.system.datatype == "table") {
            // if (!foundry.utils.hasProperty(attData[attribute], "tableitems")) {
            populate = true;
            // }
        ...
        if (property.system.datatype == "checkbox") {
            ...
            if (attData[attribute] != null)
                foundry.utils.setProperty(attData[attribute], "checkgroup", property.system.checkgroup);

        }

        //console.log(property.data.data.attKey + " " + property.data.data.datatype + " " + populate);
        if (!foundry.utils.hasProperty(attData, attribute) || Object.keys(attData[attribute]).length == 0 || populate) {
            //console.log("populating prop");
            attData[attribute] = {};
            foundry.utils.setProperty(attData[attribute], "id", "");
            attData[attribute].id = attID;

            //Sets id and auto
            if (property.system.datatype != "table") {

                if (!foundry.utils.hasProperty(attData[attribute], "value"))
                    foundry.utils.setProperty(attData[attribute], "value", "");
                foundry.utils.setProperty(attData[attribute], "prev", "");
                await foundry.utils.setProperty(attData[attribute], "isset", false);

                //Sets auto, auto max, and max
                if (property.system.automax != "" || property.system.datatype == "radio") {

                    foundry.utils.setProperty(attData[attribute], "max", "");
                ...
                foundry.utils.setProperty(attData[attribute], "istable", true);
                foundry.utils.setProperty(attData[attribute], "totals", {});
                if (!foundry.utils.hasProperty(this.actor.system.attributes[attribute], "tableitems")) {
                    foundry.utils.setProperty(attData[attribute], "tableitems", []);
                }
                ...
                for (let i = 0; i < groupprops.length; i++) {
                    ...
                    foundry.utils.setProperty(attTableKey.totals, propKey, {});
                    const tableAtt = attTableKey.totals[propKey];
                    foundry.utils.setProperty(tableAtt, "id", propId);
                    if (propData.system.totalize) {

                        foundry.utils.setProperty(tableAtt, "total", "");
                        foundry.utils.setProperty(tableAtt, "prev", "");
                    }
                    //TO FIX IN FUTURE
   ...
   */
    ...
    async addNewPanel(newHTML, tabpanel, tabKey, tabname, firstmrow, multiID = null, multiName = null, _paneldata = null, multiheadergroup = null) {
        ...
        if (tabpanel.system.title != "") {
            ...
            new_header.setAttribute('data-panel-key',tabpanel.system.panelKey);
        ...
        for (let n = 0; n < properties.length; n++) {
            ...
            if (propertybase == null) {
                ...
                if (property.system.haslabel && property.system.datatype != "table" && property.system.datatype != "badge" && property.system.datatype != "button") {
                    ...
                    let sLabelAlignment=`sb-label-alignment-${property.system.labelAlignment}`;
                    sLabel.className = labelwidth + " " + sLabelAlignment + " " + property.system.attKey + "_label";
                ...
                //Check property type
                    ...
                    let sInputAlignment=`sb-input-alignment-${property.system.inputAlignment}`;
                    sInput.className =sInput.className + " " + sInputAlignment;                
                    
               
                    sInput.setAttribute("name", "system.attributes." + property.system.attKey + ".value");
                    sInput.setAttribute("title", property.system.tooltip);                    
                    let strList=property.system.listoptions;                    
                    sInput.insertAdjacentHTML('beforeend', `{{selectOptions (parseListOptions "` + strList + `") selected=actor.system.attributes.` + property.system.attKey + `.value valueAttr="key" labelAttr="label" }}`); 
                }
                        ...
                        if (group != null && property.system.tableoptions.showGroupProperties) {
                            ...
                            for (let i = 0; i < groupprops.length; i++) {
                                ...
                                if(!firstColumnAssigned){
                                  ...
                                  let sLabelAlignment=`sb-label-alignment-center`;
                                  hCell.classList.remove("centertext");
                                  hCell.classList.add(sLabelAlignment)
                                }
                    ...
                    if (property.system.datatype === "simplenumeric") {
                        ...
                        if (!foundry.utils.hasProperty(property.system, "maxvisible")) {
                        ...
                        if (property.system.automax != "" && property.system.maxvisible) {
                            ...
                            let sInputAlignment=`sb-input-alignment-${property.system.inputAlignment}`;
                            sInputMax.className += " " + sInputAlignment;
                ...
                let sInputAlignment=`sb-input-alignment-${property.system.inputAlignment}`;
                sInput.className += " " + sInputAlignment;
                //Adds identifier
   ...
   */
    ...
    async buildSheet() {
        ...
        foundry.utils.setProperty(flags, "multiwidth", 0);
        foundry.utils.setProperty(flags, "maxwidth", 0);
        foundry.utils.setProperty(flags, "maxrows", 0);
        foundry.utils.setProperty(flags, "multiwclass", "");
   ...
   */
    async _onDrop(event) {
        ...
        //Add tab id to panel
        let subitems = foundry.utils.duplicate(this.actor.system[subitemsTag]);
    ...
    async refreshCItems(basehtml) {
                        ...
                        if ((propTable.system.onlynames != "ONLY_NAMES")) {
                            ...
                            if(propTable.system.tableoptions.showGroupProperties){
                                ...
                                for (let k = 0; k < groupprops.length; k++) {
                                    ...
                                    let sCellAlignment=`sb-label-alignment-center`;
                                    if (propdata.datatype === "label" || propdata.datatype === "simpletext" || propdata.datatype === "list" || propdata.datatype === "simplenumeric" || propdata.datatype === "checkbox" || propdata.datatype === "radio"){                                      
                                    ...
                                    if (((ciObject.attributes[propKey] != null && propdata.datatype != "label") || (propdata.datatype == "label")) && !propdata.ishidden) {
                                        if (propdata.datatype == "textarea") {
                                            ...
                                            let textareatooltip="";
                                            if (isFree) {
                                              textareatooltip=ciObject.attributes[propKey].value; 
                                            } else {
                                              textareatooltip=ciObject.attributes[propKey].value; 
                                            }
                                            
                                            textSymbol.setAttribute('data-tooltip',textareatooltip);
                                            ...
                                            if (isconstant) {
                                                ...
                                                if (propdata.datatype == "label") {
                                                    if (propdata.labelformat == "D" || propdata.labelformat == "I") {
                                                        ...
                                                        let sLabelAlignment=`sb-label-alignment-center`;
                                                        new_cell.classList.remove("centertext");
                                                        new_cell.classList.add(sLabelAlignment)
                                                    ...
                                                    if (propdata.datatype === "checkbox") {
                                                        ...
                                                        cellvalue.setAttribute('data-property-check-value',setvalue);
                                                        cellvalue.setAttribute("disabled", "disabled");
                                                        cellvalue.disabled=true;
                                                        if (propdata.customcheck && propdata.onPath != "" && propdata.offPath != "") {
                                                          cellvalue.className += " " + 'customcheck-for-tables';
                                                          cellvalue.setAttribute("onPath", propdata.onPath);
                                                          cellvalue.setAttribute("offPath", propdata.offPath);
                                                        }
                                                        //console.log("lol");
                                                      ...
                                                      new_cell.appendChild(cellvalue);
                                                        ...
                                                        //console.log(propdata.datatype,constantvalue,propdata);
                                                        let sLabelAlignment=`sb-label-alignment-center`;
                                                        new_cell.classList.remove("centertext");
                                                        new_cell.classList.add(sLabelAlignment)
                                                        new_cell.textContent = cContent;
                                                ...
                                                if (propdata.datatype === "checkbox") {
                                                    ...
                                                    let sInputAlignment=`sb-input-alignment-${propdata.inputAlignment}`;
                                                    cellvalue.classList.add(sInputAlignment);
                                                    ...
                                                    cellvalue.className = "table-input";
                                                    if (propdata.datatype === "simpletext"){
                                                      let sInputAlignment=`sb-input-alignment-${propdata.inputAlignment}`;
                                                      cellvalue.className =cellvalue.className + " " + sInputAlignment; 
                                                    }
                                                    if (propdata.datatype === "label"){
                                                      let sLabelAlignment=`sb-label-alignment-${propdata.labelAlignment}`;
                                                      cellvalue.className =cellvalue.className + " " + sLabelAlignment; 
                                                    }
                                                ...
                                                else
                                                {
                                                  ...
                                                  if (propdata.datatype != "checkbox") {
                                                      ...
                                                      if(propdata.datatype === "simpletext"){
                                                        cellvalue.setAttribute("data-tooltip", cellvalue.value);
                                                      }
                                                      ...
                                                      cellvalue.checked = setvalue;
                                                      cellvalue.setAttribute('data-property-check-value',setvalue); //data-property-value
      ...
      //                                            }
                ...
                if (table != null && (groupcitems.length == 0 || (tableHasValidFilter!=null && filter_passed_count==0 ))) {
                    ...
                    if (headerRow ==null){
                      // most likely, the table does not have any data
                      //console.log(`Sandbox | Table header not found for key [${propTable.system.attKey}]`);                    
                    }
                    else{
                          ...
                          for (let k = 0; k < groupprops.length; k++) {
                            ...
                            if (!propdata.ishidden) {
                                if (propdata.totalize) {
                                    ...
                                    total_cell.classList.remove('sb-label-alignment-left');
                                    total_cell.classList.remove('sb-label-alignment-right');
                                    total_cell.className += ` sb-label-alignment-center`;
...
<input class="check-transfer-consumed-first" type="checkbox" id="transfer-consumed-first" checked>
            ...
            buttons: {
                one: {
                    ...
                    callback: async (html) => {
                        ...
                        let newcitems = foundry.utils.duplicate(this.actor.system.citems);
                ...
                one: {
                    ...
                    callback: async (html) => {
                        if (!disabled) {
                            ...
                            let freeattributes = foundry.utils.duplicate(this.actor.system.attributes[freeTableKey].tableitems);
    ...
    async saveNewCIAtt(ciId, propId, propKey, value) {
        ...
        let cItemsID = foundry.utils.duplicate(this.actor.system.citems);
    ...
    async saveNewFreeItem(id, tableKey, fpropKey, value, ischeck = false, checked = null) {
        let myfreeItems = await foundry.utils.duplicate(this.actor.system.attributes[tableKey].tableitems);
    ...
    async useCIIcon(itemId, ciKey, value, iscon = false, isactivation = false) {
        //console.log('Sandbox | useCIIcon');
        
        let citemObjfinder = await auxMeth.getcItem(itemId, ciKey);
        const citemObj = citemObjfinder.system;
        if(citemObj.requiresTarget && (citemObj.usetype=='CON')){
          let targets = game.user.targets.ids; // array of id
          if (targets.length==0){
            let msg=game.i18n.format("SANDBOX.cItem.Msg.CanNotActivatecItemNoTargetSelected", {actorname:this.actor.name,citemname: citemObjfinder.name});
    ...
    async activateCI(itemId, value, iscon = false, roll = null, isactivation = false) {
        //console.log('Sandbox | activateCI');        
        
        const actorData = foundry.utils.duplicate(this.actor.system);
        ...
        let objectUses = foundry.utils.duplicate(citem.uses);
    ...
    async rechargeCI(itemId, ciKey) {
        const citems = foundry.utils.duplicate(this.actor.system.citems);
        const citem = citems.find(y => y.id == itemId);
    ...
    async changeCINum(itemID, ciKey, value) {
        ...
        let citemIDs = foundry.utils.duplicate(this.actor.system.citems);
  ...
  async setCheckboxImages(basehtml) {
    let html = await basehtml.find(".customcheck");
    for (let i = 0; i < html.length; i++) {
    ...
    // customcheck-for-tables
    // checkboxes in tables
    html = await basehtml.find(".customcheck-for-tables");
    for (let i = 0; i < html.length; i++) {
      let checkNode = html[i];
      let onPath = checkNode.getAttribute("onPath");
      let offPath = checkNode.getAttribute("offPath");
      let checkValue = checkNode.getAttribute("data-property-check-value");     
        let selected = offPath;
    ...
    async setTokenOptions(myactorData, path = null) {
        if (path == null)
            path = this.actor.img;
        let mytoken = await foundry.utils.duplicate(this.actor.prototypeToken);
