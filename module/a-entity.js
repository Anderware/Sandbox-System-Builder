...
export class gActor extends Actor {
    ...
    async createEmbeddedDocuments(embeddedName,data,operation){                  
      if (data[0].type=="cItem"){
        //console.log("Sandbox | abort createEmbeddedDocuments");
        // dont embedd this, give the actor it the sandbox way
        const ciKey=data[0].system.ciKey;
        const id=ciKey;
        // get the citem form items
        let citem = await auxMeth.getcItem(id, ciKey); 
        if (citem!=null){
          let api=game.system.api;
          api.ActorcItem_Add(this,citem, 1);
        } else{
          console.log("Sandbox | createEmbeddedDocuments can not find cItem[" + data[0].name + "] ciKey[" + ciKey + "]" );
        }
        // abort the embedd
        return null;
      }
    ...
    prepareData() { // v10
        ...
        if (!foundry.utils.hasProperty(flags, "ischeckingauto")) {
            foundry.utils.setProperty(flags, "ischeckingauto", false);
        }
        if (!foundry.utils.hasProperty(flags, "hasupdated")) {
            foundry.utils.setProperty(flags, "hasupdated", true);
        }
        ...
        if (data.istemplate) {
            if (!foundry.utils.hasProperty(flags, "tabarray")) {
                foundry.utils.setProperty(flags, "tabarray", []);
            }
            if (!foundry.utils.hasProperty(flags, "rows")) {
                foundry.utils.setProperty(flags, "rows", 0);
                foundry.utils.setProperty(flags, "rwidth", 0);
            }
        }
        if (!foundry.utils.hasProperty(flags, "sandbox")) {
            foundry.utils.setProperty(flags, "sandbox", {});
        }
        if (!foundry.utils.hasProperty(flags.sandbox, "scrolls_" + game.user.id + "_" + this.id)) {
            foundry.utils.setProperty(flags.sandbox, "scrolls_" + game.user.id + "_" + this.id, 0);
        }
        //console.log(this);
    }

    prepareDerivedData() {  // v10  
        if (!foundry.utils.hasProperty(this.flags, "sbupdated")) {
            foundry.utils.setProperty(this.flags, "sbupdated", 0);
        }
        if (!foundry.utils.hasProperty(this.system, "biovisible")) {
            foundry.utils.setProperty(this.system, "biovisible", false);
        }
    }
    ...
    async _preUpdate(updateData, options, userId) { //v10
        //        let upclon = foundry.utils.duplicate(updateData);
        ...
        if (!this.system.istemplate && updateData.system != null && noTemplate) {
            ...
            let actor = foundry.utils.duplicate(this);
            let newtoken;
            //console.log(actor);
            ...
            foundry.utils.setProperty(maindata, "selector", adata.selector);
            if (newattributes)
                foundry.utils.setProperty(maindata, "attributes", newattributes);
            if (newcitems.length > 0) {
                foundry.utils.setProperty(maindata, "citems", newcitems);
            }
            else {
            ...
            if (newrolls)
                foundry.utils.setProperty(maindata, "rolls", newrolls);
        }
        // check for deleted attributes
    ...
    async getFinalActorData(actor, updateData) { //v10
        ...
        for (var key in attributes) {
            if (attKeys.includes(key)) {
                ...
                if (actorData.attributes[key].istable && foundry.utils.hasProperty(attributes[key], "tableitems"))
                    actorData.attributes[key].tableitems = attributes[key].tableitems;
                if (actorData.attributes[key].istable && foundry.utils.hasProperty(attributes[key], "totals")) {
    ...
    async updateModifiedData(originaldata, extradata) { //v10
        let existingData = await foundry.utils.duplicate(originaldata);
    ...
    async update(data, options = {}) {
        ...
        if (data != null) {
            if (data["citems"] != null) {
                newdata = {};
                foundry.utils.setProperty(newdata, "data", {});
        ...
        if (this.system.gtemplate != null) {
            if (newdata == null) {
                foundry.utils.setProperty(newdata, "flags", {});
            }
            if (!newdata.flags) {
                newdata["flags.sandbox." + "scrolls_" + game.user.id + "_" + this.id] = scrollTop;
            }
            else {
                if (!foundry.utils.hasProperty(newdata.flags, "sandbox"))
                    foundry.utils.setProperty(newdata.flags, "sandbox", {});
                newdata.flags.sandbox["scrolls_" + game.user.id + "_" + this.id] = scrollTop;
            }
        }
        //console.log(newdata);
    ...
    async addcItem(ciTem, addedBy = null, data = null, number = null, uses = null,addedType='USER') {
        ...
        let newdata = foundry.utils.duplicate(data);
        ...
        if (!foundry.utils.hasProperty(ciTem.system.groups)) {
        ...
        //let itemKey = "";
        let itemKey = ciTem.id;
        let newItem = {};
        //console.log(ciTem.data.data.groups);
        foundry.utils.setProperty(newItem, itemKey, {});
        ...
        if (!data.system.istemplate) {
            ...
            newItem[itemKey].attributes = await foundry.utils.duplicate(citemData.attributes);
    ...
    async deletecItem(itemID, cascading = false, thisData = null) {
        ...
        let newdata;
        if (thisData == null) {
            newdata = foundry.utils.duplicate(this);
        }
        else {
            newdata = thisData;
        }
    ...
    async updateCItems() {
        ...
        for (let i = 0; i < citems.length; i++) {
            ...
            if (citemTemplate != null && foundry.utils.hasProperty(citemTemplate.system, "groups")) {
    ...
    async checkAttConsistency(attributes, mods) {
        ...
        for (let i = 0; i < attArray.length; i++) {
            ...
            if (property != null) {
                ...
                if (!foundry.utils.hasProperty(attributes, attribute)) {
                    //console.log("noatt");
                    await foundry.utils.setProperty(attributes, attribute, {});
                }

                if (!foundry.utils.hasProperty(attributes[attribute], "id")) {

                    await foundry.utils.setProperty(attributes[attribute], "id", property.id);
                    attID = property.id;
                ...
                if (!foundry.utils.hasProperty(attributes[attribute], "value")) {
                    //console.log("novalue");
                    await foundry.utils.setProperty(attributes[attribute], "value", defvalue);
                }

                if (!foundry.utils.hasProperty(attributes[attribute], "max")) {
                    //console.log("nomax");
                    await foundry.utils.setProperty(attributes[attribute], "max", "");
                }

                if (!foundry.utils.hasProperty(attributes[attribute], "prev")) {
                    //console.log("noprev");
                    await foundry.utils.setProperty(attributes[attribute], "prev", defvalue);
                }
            }
    ...
    async getMods(citemIDs) {
        ...
        for (let n = 0; n < citemIDs.length; n++) {
            ...
            if (citemObjBase != null) {
                ...
                if (!foundry.utils.hasProperty(citemObj, "ciKey"))
                    citemObj = citemObjBase._source.system;

                for (let i = 0; i < citemObj.mods.length; i++) {
                    let toaddMod = foundry.utils.duplicate(citemObj.mods[i]);
                    toaddMod.citem = ciID;
    ...
    async execITEMmods(mods, data) {         // data argument is an actor
        ...
        for (let i = 0; i < itemmods.length; i++) {
            ...
            //console.log(_citem);
            if (!foundry.utils.hasProperty(_citem, "ciKey"))
            ...
            if (mod.selectnum == 0) {
                ...
                if (!jumpmod) {
                    ...
                    if (!foundry.utils.hasProperty(citem, "selection")) {
                        foundry.utils.setProperty(citem, "selection", []);
                    }
    ...
    async checkcItemConsistency(actorData) {
        ...
        for (let k = 0; k < citemIDs.length; k++) {
            ...
            let cITemplate = await foundry.utils.duplicate(cIOrigTemplate);
            ...
            for (let j = 0; j < cITemplate.system.groups.length; j++) {
                ...
                for (let j = 0; j < groupProps.length; j++) {
                    ...
                    if (tempAtt != null) {
                        ...
                        if (foundry.utils.hasProperty(mycitem.attributes, att)) {
                        ...
                        if (game.user.isGM) {
                            foundry.utils.setProperty(cITemplate.system.attributes, att, {});
                    ...
                    if (!foundry.utils.hasProperty(mycitem.attributes, att)) {
                        if (this.ownership.default >= CONST.DOCUMENT_OWNERSHIP_LEVELS.OBSERVER || this.ownership[game.user.id] >= CONST.DOCUMENT_OWNERSHIP_LEVELS.OBSERVER || game.user.isGM) {
                            foundry.utils.setProperty(mycitem.attributes, att, {});
                            foundry.utils.setProperty(mycitem.attributes[att], "value", newvalue);
                        }
    ...
    async checkPropAuto(actorData, repeat = false) {
        ...
        for (let attribute in attributes) {
            ...
            await foundry.utils.setProperty(attdata, "isset", false);
            await foundry.utils.setProperty(attdata, "maxset", false);
            await foundry.utils.setProperty(attdata, "default", false);

            //TEST TO DELETE
            await foundry.utils.setProperty(attdata, "autoadd", 0);
            await foundry.utils.setProperty(attdata, "maxadd", 0);
            await foundry.utils.setProperty(attdata, "maxexec", false);
        ...
        const citemIDs = actorData.system.citems;
        let originalcIDs = foundry.utils.duplicate(citemIDs);
        //console.log(citemIDs);
        ...
        for (let i = 0; i < createmods.length; i++) {
            ...
            if (!foundry.utils.hasProperty(attributes, modAtt)) {
                foundry.utils.setProperty(attributes, modAtt, {});
                foundry.utils.setProperty(attributes[modAtt], "id", mod.citem + "_" + mod.index);
                foundry.utils.setProperty(attributes[modAtt], "value", mod_defvalue);
                foundry.utils.setProperty(attributes[modAtt], "prev", mod_defvalue);
                foundry.utils.setProperty(attributes[modAtt], "autoadd", 0);
                foundry.utils.setProperty(attributes[modAtt], "isset", false);
                foundry.utils.setProperty(attributes[modAtt], "created", true);
                foundry.utils.setProperty(attributes[modAtt], "hastotals", false);
            }
            ...
            if (property != null) {
                ...
                if (property.system.automax != null) {
                    if (property.system.automax != "") {
                        ...
                        if (!foundry.utils.hasProperty(attdata, "maxblocked"))
                            attdata.maxblocked = false;
                ...
                if (foundry.utils.hasProperty(attributes, modAtt)) {
                    ...
                    if (checker) {
                        ...
                        if (_basecitem != null) {
                            const _mod = await _basecitem.mods.find(x => x.index == mod.index);
                            if(_mod.expr == null){
                              console.warn(`Sandbox | Bad expression in mod[${mod.index}] for cItem [${_basecitem.name}]`);
                            }
        ...
        for (let roll in rolls) {
            ...
            foundry.utils.setProperty(rolls[roll], "value", "");
        }
        ...
        for (let i = 0; i < rollmods.length; i++) {
            ...
            if (!jumpmod) {
                if (!foundry.utils.hasProperty(rolls, rollID)) {
                    foundry.utils.setProperty(rolls, rollID, {});
                    foundry.utils.setProperty(rolls[rollID], "value", "");
                    //ithaschanged = true;
                }
        ...
        if (expArray != null) {
            for (let i = 0; i < expArray.length; i++) {
                ...
                if (target != null) {
                    ...
                    ui.notifications.warn("No target found for " + mode + "()");
                    continue;
                }
            }
        }
    ...
    async rollSheetDice(rollexp, rollname, rollid, actorattributes, citemattributes, number = 1, isactive = null, ciuses = null, cimaxuses = 1, target = null, rollcitemID = null, tokenID = null) {
        ...
        if (rollexp.includes("~requirestarget~")){
          let targets = game.user.targets.ids; // array of id
          if (targets.length==0){
            let msg=game.i18n.format("SANDBOX.roll.Msg.CanNotActivateRollNoTargetSelected", {actorname:this.name,rollname: rollname});
...
/// IS THIS A REAL ASSIGN(=)? Should it not be (==) ?????
            ...
            if (parseInt(sRoll.numdice) > 0) {
                ...
                let finalroll = await partroll.evaluate();
...
/// IS THIS A REAL ASSIGN(=)? Should it not be (==) ?????
            ...
            if (blocks[2] != null) {
                ...
                for (let k = 0; k < rollpid.length; k++) {
                    if (rollpid[k] != "" && foundry.utils.hasProperty(actorrolls, rollpid[k])) {
                ...
                for (let j = 0; j < currentDice.length; j++) {
                    ...
                    for (let m = 0; m < diceMods.length; m++) {
                        if (diceMods[m].includes("im")) {
                            ...
                            let impRollFinal = await impRoll.evaluate();
                            if (game.dice3d != null && !nochat) { //Dice So Nice Module
        ...
        for (let k = 0; k < rollid.length; k++) {
            if (rollid[k] != "" && foundry.utils.hasProperty(actorrolls, rollid[k])) {
        ...
        let partroll;
        try{
          partroll = new Roll(rollexp);
        }
        catch(err)
        {
          console.warn(`Sandbox | Error in Roll expression: ${rollexp}`);
          console.warn(err.message);
          return null;
        }
        
        roll = await partroll.evaluate();
        if (game.dice3d != null && !nochat) {
        ...
        for (let j = 0; j < multiroll.length; j++) {
          ...
          if (!foundry.utils.hasProperty(multiroll[j], "extraroll") && multirolldice.length > 0) {
              if (rolldice == null) {
        ...
        if (conditionalText != "") {
            ...
            for (let i = 0; i < blocks.length; i++) {              
                ...
                //console.log('conditionalText:' + thiscond);
                if (thiscond.length > 1) {                  
                  // Sanbox 0.16
                  let condblocks = thiscond.split(";");
                  let compareValue = condblocks[0];
                  let conditions    = condblocks[1];

                  if(compareValue==='total')
                  {
                    compareValue = rolltotal; 
                  }
                  else
                  {
                    compareValue = await auxMeth.autoParser(compareValue, actorattributes, citemattributes, false, false, number);
                  }
                  conditions = await auxMeth.autoParser(conditions, actorattributes, citemattributes, false, false, number);
                  //console.log(`compareValue[${compareValue}]`);
                  //console.log(`conditions[${conditions}]`);

                  let finalevalvalue=auxMeth.GetConditionalText(compareValue,conditions, actorattributes, citemattributes, number);
        ...
        if (!nochat) {
            let newhtml = await foundry.applications.handlebars.renderTemplate("systems/sandbox/templates/dice.html", rollData);
            let rtypevalue = '';
            // check if any roll mode flags have been set by roll expression/ids
            if (gmmode || blindmode || selfmode) {
                if (gmmode) {
            ...
            let rvalue = CONST.CHAT_MESSAGE_STYLES.OTHER;
            switch (rtypevalue) {      //roll, gmroll,blindroll,selfroll
                case CONST.DICE_ROLL_MODES.PUBLIC:
                    rvalue = CONST.CHAT_MESSAGE_STYLES.IC;
                    break;
                case CONST.DICE_ROLL_MODES.PRIVATE:
                    rvalue = CONST.CHAT_MESSAGE_STYLES.WHISPER;
                    break;
                case CONST.DICE_ROLL_MODES.BLIND:
                    rvalue = CONST.CHAT_MESSAGE_STYLES.WHISPER;
                    blindmode = true;
    ...
    sendMsgChat(flavor, msg, submsg) {
        ...
        foundry.applications.handlebars.renderTemplate("systems/sandbox/templates/msg.html", rollData).then(html => {
            ChatMessage.create({
                content: html
            });
    ...
    async handleTransferEdit(data) {
        ...
        let ownercItems = foundry.utils.duplicate(actorOwner.system.citems);
        ...
        let receivercItems = foundry.utils.duplicate(actorReceiver.system.citems);
