...
async function setupCustomCSS(){  
  if (game.modules.get("custom-css")!=null){
    try{
      ...
      const customcsssrc='/modules/custom-css/ccss-editor.js';
      const mymodule = await import(customcsssrc);
      const foo = mymodule.default; // Default export
      const SettingsForm = mymodule.CcssEditor; // Named export
...
Hooks.once("init", async function () {
    ...
    foundry.applications.ux.ContextMenu.eventListeners();
    ...
    foundry.documents.collections.Actors.registerSheet("dnd5e", gActorSheet, { makeDefault: true });
    foundry.documents.collections.Items.unregisterSheet("core", foundry.appv1.sheets.ItemSheet);
    foundry.documents.collections.Items.registerSheet("dnd5e", sItemSheet, { makeDefault: true });
...
Hooks.once('ready', async () => {
    ...
    if (game.user.isGM) {
        ...
        incDC.addEventListener("click", async (event) => {
          ...
          // #dcroll-bar > div.dc-input > form > input
          const newValue=Number(sInput.value) + change;
          sInput.value = newValue;
          //sInput.focus();
          //sInput.setSelectionRange(newValue.length, newValue.length);  // Clear selection, place cursor at end
          await game.settings.set("sandbox", "diff", Number(newValue));
        });
        ...
        sInput.setAttribute("id", "dcinput");
        ...
        if (game.settings.get("sandbox", "showDC")) {
          ...
          if(menuItems.length>0){
            ...
            new DropDownMenu($(".dc-header"), `#sb-dc-selector`, menuItems,dropDownMenuOptions);            
          }
        }
  ...
  if (app.tabName == "settings") {
    ...
    if (infoSection != null) {      
      try {
        ...
        let menuItems = [
          {
            ...
            tooltip: "SANDBOX.SystemMenu.BuildActorTemplates.Tooltip",
            condition: true,
            callback: async() => {
          ...
          },
          {
            ...
            icon: "<i class='fas fa-arrow-alt-circle-down fa-fw'></i>",
            tooltip: "SANDBOX.SystemMenu.ReloadActors.Tooltip",
            condition: true,
            callback: async() => {
              // get user confirmation to proceed
          ...
          },
          {
            ...
            tooltip: "SANDBOX.SystemMenu.CheckConsistency.Tooltip",
            condition: true,
            callback: async() => {
              // get user confirmation to proceed
        ...
        let sandboxSection = document.createElement("section");
        sandboxSection.className = 'sandbox flexcol';
        sandboxSection.innerHTML = `<h4 class="divider">${game.system.title}</h4>`;
                                                
        let sandboxquickmenu = document.createElement("UL");
        sandboxquickmenu.className="sb-menu-bar-container";
        ...
        sandboxquickmenu.append(htmlmenubar);
        sandboxSection.append(sandboxquickmenu);

        infoSection.after(sandboxSection);
      } catch (err) {
        console.error(err.message);
      }

      //#settings > section.info.flexcol
...
Hooks.on("renderSidebarTab", createShortcuts); // works in v12, not v13

function createShortcuts(sidebar, jq) {
    console.log("Sandbox | createShortcuts");
...
//    console.log('preUpdateActor - actor',actor);
    ...
    if (game.user.isGM && actor.getFlag('sandbox','originalId') == null ) {
      console.log('Sandbox | preUpdateActor | ' + 'Updating originalId for actor ['+ actor.name +']');
      if (!foundry.utils.hasProperty(updateData, "flags")){                     
        // update carries no flags
        foundry.utils.setProperty(updateData, "flags", {'sandbox':{'originalId':actor.id}});
      } else {
        // update has flags        
        if (!foundry.utils.hasProperty(updateData.flags, "sandbox")){
    ...
    if (updateData.name) {
        if (!actor.system.istemplate) {
            if (!updateData.token)
                foundry.utils.setProperty(updateData, "token", {});
    ...
    if (updateData["system.rollmode"]) {
        if (!foundry.utils.hasProperty(updateData, "system"))
...
Hooks.on("preUpdateItem", async (item, updateData, options, userId) => {
    ...
    if (game.user.isGM && item.getFlag('sandbox','originalId') == null ) {
      console.log('Sandbox | preUpdateItem | ' + 'Updating originalId for item ['+ item.name +']');
      if (!foundry.utils.hasProperty(updateData, "flags")){                     
        // update carries no flags
        foundry.utils.setProperty(updateData, "flags", {'sandbox':{'originalId':item.id}});
      } else {
        // update has flags        
        if (!foundry.utils.hasProperty(updateData.flags, "sandbox")){
          // update carries no sandbox flags
...
Hooks.on("rendergActorSheet", async (app, html, data) => {
    ...
    if (!actor.system.istemplate) {
        ...
        app.setImages(html);
        await app.setCheckboxImages(html);
...
//        console.log(html);
    ...
    if(typeof data.message.speaker.actor==='string' ){        
      ...
      speakerimg = game.users.get(data.message.author).avatar;
    }
    let hide = false;
    ...
    let realuser = game.users.get(data.message.author);
    ...
    if (_html.includes("dice-roll") && !_html.includes("table-draw")) {  
        ...
        await foundry.applications.handlebars.renderTemplate("systems/sandbox/templates/dice.html", rollData).then(async newhtml => {
...
//            }
    ...
    if (!_html.includes("roll-template")) {
        ...
        await foundry.applications.handlebars.renderTemplate("systems/sandbox/templates/sbmessage.html", msgData).then(async newhtml => {
...
Hooks.on("renderDialog", async (app, html, data) => {
  ...
  if (app.data.citemdialog) {
    ...
    foundry.utils.setProperty(actor.flags, "selection", []);
