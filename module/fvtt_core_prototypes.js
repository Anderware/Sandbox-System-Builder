...
export function fvtt_core_prototypes(){
  ...
  foundry.documents.collections.CompendiumCollection.prototype.importAll = async function({folderId=null, folderName="", ...options}={}) {
    let parentFolder;
    ...
    for ( let n=0; n<nBatches; n++ ) {
      ...
      for (let i = 0; i < docs.length; i++) {
        let mydoc = docs[i];
        if (foundry.utils.hasProperty(mydoc, "system"))
          if (foundry.utils.hasProperty(mydoc.system, "ciKey")) {
  ...
  foundry.documents.abstract.WorldCollection.prototype.importFromCompendium = async function (pack, id, updateData={}, options={}) {
   ...
   * @private
   */  
  foundry.applications.apps.FilePicker.implementation.prototype._getExtensions=function(type) {        
