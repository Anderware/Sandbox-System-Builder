...
async function replaceAllMissingImages(){
  ...
  await replaceMissingActorImages();
  await replaceMissingTokenImages();
  // reset global flag
...
async function replaceMissingItemImages(itemtype) {
  ...
  let fileexists;
  let datatype = '';
  let items = await game.items.filter(y => (y.type == itemtype));
  for (let i = 0; i < items.length; i++) {
    ...
    if (!fileexists) {
      ...
      if (itemtype == 'property') {
        datatype = items[i].system.datatype;
        if (items[i].system.hasroll) {
          rollable = true;
        }
      }
      let iconfile = api.getSandboxItemDefaultIconFile(itemtype, datatype, rollable);
...
let iconfile=api.getSandboxItemDefaultIconFile('actor');
for (let actor of actors) {
   fileexists=await api.fileExists(actor.img);
   if(!fileexists){               
     console.log('Sandbox | Updating actor [' + actor.name + '] image');
...
async function replaceMissingTokenImages() {
  ...
  let iconfile = api.getSandboxItemDefaultIconFile('token');
  for (let scene of scenes) {
    let tokens = scene.tokens;
    for (let token of tokens) {
      fileexists = await api.fileExists(token.texture.src);
      if (!fileexists) {
        console.log('Sandbox | On scene ['+ scene.name + '], updating token [' + token.name + '] image');
        await token.update({[`texture.src`]: iconfile});
      }
    }
  }
...
async function fileExists(url){
  ...
  // look in data first
  try{
    const findInData =await foundry.applications.apps.FilePicker.implementation.browse('data',url);    
    if (findInData!=null){
      result=true;
    } 
  }
  ...
  if(!result){
    try{
      const findInData =await foundry.applications.apps.FilePicker.implementation.browse('public',url);    
      if (findInData!=null){
        result=true;
      } 
    }
...
async function BuildActorTemplates(actortemplatename=''){
  ...
  if(actortemplates.length>0){ 
    ...
    for(let actortemplate of actortemplates){  
      if(actortemplate!=null){
        ...
        try{
          ...
          console.error('Sandbox | Error building actor template' + actortemplate.name);
          console.error(err);          
          buildOk=false;
        }
      }
    ...
    if(buildOk){      
      ...
      let prompttitle = game.i18n.localize("SANDBOX.SystemMenu.BuildActorTemplatesCompleteBody.Title");
      let promptbody = game.i18n.localize("SANDBOX.SystemMenu.BuildActorTemplatesCompleteBody.Body") ;    
      let answer = await sb_custom_dialog_confirm(prompttitle, promptbody, game.i18n.localize("Yes"), game.i18n.localize("No")); 
      if (answer){
      ...
      await sb_custom_dialog_prompt(game.i18n.localize("Warning"),game.i18n.localize("SANDBOX.BuildActorTemplatesFailedBody"),game.i18n.localize("Ok"),'Warning');
    }
  }
  else{
    ui.notifications.warn('No actor template(s) found');
  }    
}

async function ReloadActorsFromTemplate(actorTemplateName = ''){
  let actors;
  if(actorTemplateName == ''){
    // reload all actors
    actors = game.actors.filter(y =>(y.system.istemplate==false));
  } else {
    // only reload actors for specific template
    actors = game.actors.filter(y =>(y.system.istemplate==false && y.system.gtemplate == actorTemplateName));
  }
  if(actors!=null){    
    console.log('Sandbox | Reloading actors from templates');
    for (let actor of actors){
      //console.log(actor.name,actor.system.gtemplate );
      await actor.sheet.setTemplate(actor.system.gtemplate, null);
    }
    console.log('Sandbox | Reloaded actors from templates');
  }  
}
...
async function ActorProperty_RemoveProperty(actor, propertyKey) {
  if (actor != null) {
    if (actor.system.attributes.hasOwnProperty(propertyKey)) {
      await actor.update({[`system.attributes.-=${propertyKey}`]: null});
      console.log(`ActorProperty_RemoveProperty | Removed actor property ${propertyKey} from actor ${actor.name}`);
    }
  }
}
...
async function ActorcItem_Add(actor,citem,number=1){
  ...
  if(actorcitem==null){
    ...
    if(number>1){
      ...
      let citemIDs = foundry.utils.duplicate(actor.system.citems);
